...
Chain.prototype.defer=function(a){var b,c;a instanceof Array||(a=[a]);for(b=0;b<a.length;b++)c={type:Chain.type.ASYNC,fn:a[b]},this.$elements.push(c);return this};Chain.prototype.when=function(a){var b,c=this.$elements,d=c[c.length-1],f=Chain.type.CHAIN;a instanceof Array||(a=[a]);for(i=0;i<a.length;i++)d&&d.type==f||0<i?(d=c[c.length-1],d.chains.push(a[i])):(b={type:f,chains:[a[i]]},this.$elements.push(b));return this};
...
Chain.prototype.$done=function(a){this.$results.length&&this.$result&&(this.$results.push(this.$result),this.$result=this.$results,this.$results=[]);a(this.$result);this.$running=!1;for(a=0;a<this.$listeners.length;a++)this.$listeners[a](void 0);this.$listeners=[]};Chain.prototype.$next=function(a,b){var c=this.$elements[a],d=this.constructor.handler;if(a++>=this.$elements.length)return this.$done(b);d[c.type](this,c,a,b)};Chain.prototype.$noop=function(){};
